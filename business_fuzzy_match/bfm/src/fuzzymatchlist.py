...
class FuzzyList(object):
    ...
    def get_top_n_matches(self, query, n=5):
        """
        Creates a sub-list to search for fuzzy matches of +- 1 word length in query string.
        Searches sublist for the top 20 matches.
        """
        count, _ = self.get_broken_string(query)
        search_ind = [x for x in self.broken_length_dict.keys() if np.abs(count - x) <= 1]
        search_list = []
        for val in search_ind:
            search_list = search_list + self.broken_length_dict[val]
