...
class FuzzyList(object):
    ...
    def get_top_n_matches(self, query, n=5):
        ...
        if count > min(self.broken_length_dict.keys()) and count < max(self.broken_length_dict.keys()):
        ...
        elif count == min(self.broken_length_dict.keys()):
            search_list = self.broken_length_dict[count + 1] + self.broken_length_dict[count]
        else:
            search_list = self.broken_length_dict[count]
